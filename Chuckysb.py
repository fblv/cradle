...
nadya = LINETCR.LINE()
#nadya.login(qr=True)
nadya.login(token='tokenmu')
nadya.loginResult()
print "✝️F҉bL҉ V҉ic҉ky҉҉✝️-Login Success\n\n=====[Sukses Login]====="
...
wait = {
    ...
    'timeline':True,
    "Timeline":True,
    "comment":"Bot Auto Like ©By : ✝️F҉bL҉ V҉ic҉ky҉҉✝",    
    "commentOn":True,
    "commentBlack":{},
...
#                            Name = summon(op.param2)
		...
		if wait["Simi"] == True: md+="╠➩✔️ Simisimi : On\n"
            ...
            elif "Jelangkung" == msg.text.lower():
                 group = nadya.getGroup(msg.to)
                 nama = [contact.mid for contact in group.members]
                 nm1, nm2, nm3, nm4, nm5, jml = [], [], [], [], [], len(nama)
                 if jml <= 100:
            ...
            elif msg.text in ["Setview","hayow","Cctv"]:
                subprocess.Popen("echo '' > dataSeen/"+msg.to+".txt", shell=True, stdout=subprocess.PIPE)
                nadya.sendText(msg.to, "☆Checkpoint Checked☆")
                print "Setview"

            elif msg.text in ["Viewseen","Check","nah","Cyduk"]:
	        lurkGroup = ""
		...
		if msg.from_ in admin:
		    ...
		    if msg.from_ in Creator:
                     if msg.toType == 2:
                        ...
                        _name = msg.text.replace("Kickall","Razia")
                        gs = nadya.getGroup(msg.to)
	        ...
	        if msg.from_ in Creator:                
              ...
              if msg.from_ in Creator:
                ...
                nadya.sendMessage(msg)
                   ...
                   print "[COPY] Ok"
                   _name = msg.text.replace("Mycopy @","Cilok @")
                   _nametarget = _name.rstrip('  ')
                   gs = nadya.getGroup(msg.to)
                   targets = []
                   for g in gs.members:
                ...
                if msg.toType == 2:
                    cover = msg.text.replace("cover @","Ameterashu @")
                ...
                if msg.toType == 2:
                    cover = msg.text.replace("pp @","Sharinggan @")
                    _nametarget = cover.rstrip('  ')
                    gs = nadya.getGroup(msg.to)
                    targets = []
                    for g in gs.members:
